...
where
    T: Engine + Debug,
{
    let mut search_criteria = SetFilter::criteria(pagination, DEFAULT_SET_PAGE_SIZE);

    if let Some(name) = name {
        search_criteria.add(SetFilter::Name(name));
    }

    if let Some(entity_text) = entity_text {
        search_criteria.add(SetFilter::EntityText(entity_text));
    }

    if let Some(identifiers) = identifiers {
        search_criteria.add(SetFilter::Identifiers(identifiers));
    }
